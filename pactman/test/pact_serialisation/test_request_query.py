...
def test_v2():
    ...
    pact.given("the condition exists").upon_receiving("a request").with_request(
        "GET", "/path", query="fields=first,second"
...
@pytest.mark.parametrize("query_field", ["first,second", ["first,second"]])
def test_v3(query_field):
    pact = Consumer("consumer").has_pact_with(Provider("provider"), version="3.0.0")
    pact.given([{"name": "the condition exists", "params": {}}]).upon_receiving(
        "a request"
...
def test_matcher_in_query():
    ...
    assert target.json("3.0.0") == {
        ...
        "matchingRules": {
            "query": {
                "q": {"matchers": [{"match": "type"}]},
                "l": {"matchers": [{"match": "regex", "regex": r"\d+"}]},
            }
        },
    }
