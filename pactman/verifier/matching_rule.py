...
class Matcher:
    ...
    def get_matcher(cls, path, rule):
        ...
        if type_name not in cls.REGISTRY:
            raise RuleFailed(path, f'Invalid match type in contract {rule["match"]!r}')
...
class MultipleMatchers(Matcher):
    ...
    def apply(self, data, spec, path):
        ...
        for rule in self.rule:
            ...
            matcher = Matcher.get_matcher(path, rule)
            try:
...
def rule_matchers(rules):
    """Get all the rule matchers for the rules sets in a pact's ruleMatchers (passed in as "rules").

    Returns a dict with lists of Matcher subclass instances (e.g. MatchType) for each of query, header and body.
    """
    matchers = {}
    if 'path' in rules:
    ...
    for section in ['query', 'header', 'body']:
        if section in rules:
            matchers[section] = [Matcher.get_matcher(*i) for i in rules[section].items()]
    return matchers
