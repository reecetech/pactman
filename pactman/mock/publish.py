...
class Publisher:
    ...
    def publish_pact(self, pact, version, tags):
        ...
        tagger = self.nav['latest-version'](pacticipant=pact['consumer'])['pacticipant']['version-tag']
        for tag in tags:
            tagger(tag=tag, version=version).upsert({})

        self.nav['publish-pact'](consumer=pact['consumer'], provider=pact['provider'], version=version)
